...
$(document).ready(function() {  
	...
	$.getJSON('<?=base_url()?>api/scs/scs_today_smoke', function(data) {
		var option2 = {
			chart: {
        ...
        function requestSmoke() {
            $.ajax({
                url: '<?=base_url()?>api/scs/scs_smoke',
	...
	$.getJSON('<?=base_url()?>api/scs/scs_today_temperature', function(data) {
		var option1 = {
			chart: {
				renderTo: 'temp-chart',
				defaultSeriesType: 'spline',
        ...
        function requestTemp() {
    	    $.ajax({
    	        url: '<?=base_url()?>api/scs/scs_temperature',
    	        success: function(point) {
